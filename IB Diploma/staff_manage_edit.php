...
if (isActionAccessible($guid, $connection2, "/modules/IB Diploma/staff_manage_edit.php")==FALSE) {
	...
	if ($ibDiplomaCASStaffID=="Y") {
		...
		if ($result->rowCount()!=1) {
			...
			$row=$result->fetch() ;
			?>
			<form method="post" action="<?php print $_SESSION[$guid]["absoluteURL"] . "/modules/" . $_SESSION[$guid]["module"] . "/staff_manage_editProcess.php?ibDiplomaCASStaffID=$ibDiplomaCASStaffID" ?>">
					...
					<tr>
						...
						<td class="right">
							<select name="role" id="role" style="width: 302px">
								...
								<option <?php if ($row["role"]=="Advisor") {print "selected ";}?>value="Advisor">Advisor</option>
							</select>
							<script type="text/javascript">
