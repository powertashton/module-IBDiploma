...
if (isActionAccessible($guid, $connection2, "/modules/IB Diploma/cas_student_interview2.php")==FALSE) {
	...
	if (enroled($guid, $_SESSION[$guid]["gibbonPersonID"], $connection2)==FALSE) {
		...
		print "<div class='trailHead'><a href='" . $_SESSION[$guid]["absoluteURL"] . "'>Home</a> > <a href='" . $_SESSION[$guid]["absoluteURL"] . "/index.php?q=/modules/" . getModuleName($_GET["q"]) . "/" . getModuleEntry($_GET["q"], $connection2, $guid) . "'>" . getModuleName($_GET["q"]) . "</a> > </div><div class='trailEnd'>Student: Interview 2</div>" ;
		print "</div>" ;
		print "<p>" ;
		...
		if ($resultInterview->rowCount()>1) {
			...
			if ($resultInterview->rowCount()==0) {
				...
				?>
				<form method="post" action="<?php print $_SESSION[$guid]["absoluteURL"] . "/modules/" . $_SESSION[$guid]["module"] . "/cas_student_interview2Process.php" ?>">
